class EspIrMqttPureCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this._clearLearnTimeout();
    const language = this._resolveLanguage(config.language);
    this._config = {
      ...config,
      topic_prefix: this._normalizeTopic(config.topic_prefix || "newchuangan1/ir"),
  ...
  _clearLearnTimeout() {
    if (this._learnTimeout) clearTimeout(this._learnTimeout);
    this._learnTimeout = null;
  }

  _normalizeTopic(topic) {
    return String(topic).trim().replace(/^\/+|\/+$/g, "") || "newchuangan1/ir";
  }
  ...
  async _subscribeMQTT() {
    ...
    try {
      const unsubs = [];
      for (const [topic, callback] of [[`${this._config.topic_prefix}/stored/last`, (msg) => this._handleMqttMessage(msg)], [`${this._config.topic_prefix}/status`, (msg) => this._handleStatusMessage(msg)]]) {
        unsubs.push(await this._hass.connection.subscribeMessage(callback, { type: "mqtt/subscribe", topic }));
      }
      this._unsubscribe = () => unsubs.forEach((unsubscribe) => unsubscribe?.());
  ...
  _handleStatusMessage(message) {
    this._deviceOnline = ["online", "connected", "on", "true", "1"].includes(String(message.payload).trim().toLowerCase());
    this._render();
  }
  ...
  _startLearning() {
    ...
    this._render();
    this._publish(`${this._config.topic_prefix}/learn`, "1").catch(() => { this._isLearning = false; this._render(); });
    this._clearLearnTimeout();
  ...
  _sendCode() {
    ...
    this._publish(`${this._config.topic_prefix}/send/pronto`, code)
      ...
      .catch(() => this._toast("MQTT 发布失败，请检查连接"));
  }
      ...
      <ha-card>
        <div class="header">
          ...
          <div class="status-badge">
            ...
            <span>${this._isLearning ? this._t("statusLearning") : (this._deviceOnline ? this._t("statusOnline") : this._t("statusOffline"))}</span>
          </div>
        ...
        <div class="code-window">
          <div class="code-content ${this._learnedCode ? '' : 'empty'}" id="learned-display">
            ${this._escapeHtml(this._learnedCode || this._t("noCode"))}
          </div>
        </div>
        ...
        <div class="footer-info">
          ...
          <div class="topic-path-container" id="copy-topic-area" style="cursor: pointer;">
            <div class="topic-path" id="topic-text">${this._escapeHtml(this._config.topic_prefix)}/send/pronto</div>
